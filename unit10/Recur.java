...
public class Recur {
    ...
    public static int fun3(int n) {
        ...
        if (n == 0) {
            ...
            System.out.println("returned 3* fun("+(n-1)+")");
            return 3 * fun3(n - 1);
        }
    }
    ...
    //deified returns true
    //deepfried returns false
    ...
    //two(6) returns a very precise decimal close to 0.4
    ...
    //fixed so that mystery(10)==32
    public static int mystery(int x) {
        if (x <= 1)
            return 1;
        // return mystery(x - 1) + mystery(x - 2);
         return 2 * mystery(x - 2); //this is the right one
